...
Path("/eos/user/j/jrimmer/led_data/dictionaries/tooldaq/self_trigger/" + srn + "/fine/").mkdir(parents=True, exist_ok=True)

folder = "/eos/user/j/jrimmer/led_data/dictionaries/tooldaq/self_trigger/" + srn + "/fine/"
run_file = ur.open(
...
hit_card_ids = tree['hit_mpmt_card_ids'].array()
hit_channel_ids = tree['hit_pmt_channel_ids'].array()



events = {}
bad_events = {}
for ie in range(len(hit_card_ids)):
    
    global_chans = [card_ids[ie][i]*100+channel_ids[ie][i] for i in range(len(card_ids[ie]))]
    global_hit_chans = [hit_card_ids[ie][i]*100+hit_channel_ids[ie][i] for i in range(len(hit_card_ids[ie]))]
    
    
    
    matched_indices = np.intersect1d(global_chans, global_hit_chans,return_indices=True)[1:3]
    ...
    for iw in range(min(len(matched_indices[0]),len(matched_indices[1]))):
        
        brb_idx = matched_indices[0][iw]
        hit_idx = matched_indices[1][iw]
        
        try:

            card_id = hit_card_ids[ie][hit_idx]
            channel_id = hit_channel_ids[ie][hit_idx]
            
            waveform = waveforms[ie][brb_idx]
            waveform_time_cc = waveform_times[ie][brb_idx]/ 8
        
            hit_pmt_time = hit_pmt_times[ie][hit_idx]
            hit_pmt_time_cc = hit_pmt_times[ie][hit_idx]/8
            
        except:
            continue

        try:
            hit_pmt_time = hit_pmt_times[ie][hit_dx]
        except:
            hit_pmt_time = -999999999999

        try:
            hit_pmt_charge = hit_pmt_charges[ie][hit_idx]
        except:
        ...
        if 0 < len(peak_timebins) < 5:
            ...
            t = hit_pmt_time_cc-waveform_time_cc
            amp = max(waveform)
            baseline = np.mean(waveform[:3])
